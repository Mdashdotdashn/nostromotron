...
void SynthController::Calibrate()
{
  hwParameters_.gate_ = true;
  hwParameters_.vca_ = 60;
  ...
  {
    hwParameters_.pitch_ = (i << 14) + 0x3FFF ;
    Serial.println(progress[i%16]);
    delay(2000);
    
    cv[i] = hwParameters_.pitch_;
  ...
  SLinearRegression(pitch, cv, size, &pitchSlope_, &pitchIntercept_);
}
